...
def _rms_normalize_no_weight(x: torch.Tensor, eps: float = 1e-6) -> torch.Tensor:
    return x / torch.sqrt(x.float().pow(2).mean(dim=-1, keepdim=True).clamp_min(eps))
...
class GeometryAnalyzer:
    ...
    def _apply_output_projection(self, attn_mod: Any, x: torch.Tensor) -> torch.Tensor:
        proj = self._get_attention_output_projection(attn_mod)
        parameters = getattr(proj, "parameters", None)
        try:
            param = next(parameters()) if parameters is not None else None
        except StopIteration:
            param = None
        if param is not None:
    ...
    def _compute_attn_value_metrics(
        ...
        for layer_idx, attn_mod in attn_mods.items():
            ...
            value_pre_para = _project_parallel(y_pre, self_value)
            projected_para = self._apply_output_projection(
                attn_mod,
                value_pre_para[:, None, :],
            )[:, 0, :].to(z_in.device).float()
            projected_para_norm_t = _safe_norm(projected_para)
            z_norm_t = _safe_norm(z_in)
            projected_para_norm = float(projected_para_norm_t.mean().item())
            projected_para_over_z = float((projected_para_norm_t / z_norm_t).mean().item())
            projected_para_alignment = float(
                F.cosine_similarity(projected_para, z_in, dim=-1, eps=1e-8).mean().item()
            )
            normed_z = _rms_normalize_no_weight(z_in)
            normed_z_norm_t = _safe_norm(normed_z)
            ...
            value_metrics[layer_idx] = {
                ...
                "value_pre_projected_para_norm": projected_para_norm,
                "value_pre_projected_para_over_z": projected_para_over_z,
                "value_pre_projected_para_alignment": projected_para_alignment,
            }

            for scale in (2.0, 5.0, 10.0):
                delta_z = (scale - 1.0) * projected_para
                edit_z = z_in + delta_z
                pre_delta_over_z_t = _safe_norm(delta_z) / z_norm_t
                pre_delta_over_z = float(pre_delta_over_z_t.mean().item())
                normed_edit_z = _rms_normalize_no_weight(edit_z)
                post_delta_over_normed_z_t = _safe_norm(normed_edit_z - normed_z) / normed_z_norm_t
                post_delta_over_normed_z = float(post_delta_over_normed_z_t.mean().item())
                damping_ratio = float(
                    (post_delta_over_normed_z_t / pre_delta_over_z_t.clamp_min(1e-12)).mean().item()
                )
                cos_z_edit = float(F.cosine_similarity(z_in, edit_z, dim=-1, eps=1e-8).mean().item())
                cos_normed_z_edit = float(
                    F.cosine_similarity(normed_z, normed_edit_z, dim=-1, eps=1e-8).mean().item()
                )
                scale_tag = str(int(scale))
                value_metrics[layer_idx].update(
                    {
                        f"value_pre_scale{scale_tag}_projected_delta_over_z": pre_delta_over_z,
                        f"value_pre_scale{scale_tag}_post_rms_delta_over_normed_z": (
                            post_delta_over_normed_z
                        ),
                        f"value_pre_scale{scale_tag}_rms_damping_ratio": damping_ratio,
                        f"value_pre_scale{scale_tag}_cos_z_edit": cos_z_edit,
                        f"value_pre_scale{scale_tag}_cos_normed_z_edit": cos_normed_z_edit,
                    }
                )
    ...
    def _print_sublayer_metrics(self, result: Dict[str, Any]) -> None:
        ...
        for m in sublayer_metrics:
            ...
            elif "value_pre_para_perp_ratio" in a:
                print(
                    f"  layer={l:>2} | value-pre | "
                    f"value_pre_para_perp_ratio={a.get('value_pre_para_perp_ratio', float('nan')):.6f} | "
                    f"value_pre_para_ratio={a.get('value_pre_para_ratio', float('nan')):.6f} | "
                    f"value_pre_perp_ratio={a.get('value_pre_perp_ratio', float('nan')):.6f} | "
                    f"value_pre_projected_para_over_z={a.get('value_pre_projected_para_over_z', float('nan')):.6f} | "
                    f"value_pre_scale10_projected_delta_over_z={a.get('value_pre_scale10_projected_delta_over_z', float('nan')):.6f} | "
                    f"value_pre_scale10_post_rms_delta_over_normed_z={a.get('value_pre_scale10_post_rms_delta_over_normed_z', float('nan')):.6f} | "
                    f"value_pre_scale10_rms_damping_ratio={a.get('value_pre_scale10_rms_damping_ratio', float('nan')):.6f}"
                )
            print(
