...
LAYER="${LAYER:--1}"
HEAD="${HEAD:--1}"
NUM_LAYERS_TO_PLOT="${NUM_LAYERS_TO_PLOT:-6}"
...
PY
  ...
  echo "[INFO] Installing transformers==$want for Qwen attention visualization"
...
OUTPUT_PREFIX="${OUTPUT_PREFIX:-${BASE_OUTPUT_DIR}/qwen_xsa_attention_matrix${CKPT_SUFFIX}-data${DATA_TAG}-text${TEXT_TAG}-sample${SAMPLE_TAG}-len${MAX_LENGTH}-${CHAT_TAG}-site${SITE_TAG}-dtype${DTYPE_TAG}-dev${DEVICE_TAG}-${LAYER_TAG}}"
...
args=(
  ...
  --layer "$LAYER"
  --head "$HEAD"
  --num_layers_to_plot "$NUM_LAYERS_TO_PLOT"
...
if [[ "$parallel_enabled" == true ]]; then
  echo "[INFO] Parallel attention-matrix visualization enabled across ${#GPU_IDS[@]} GPUs"
  echo "[INFO] Selected GPUs=${GPU_IDS[*]} (MIN_FREE_MEMORY_GB=$MIN_FREE_MEMORY_GB)"
  mkdir -p representation-analysis/outputs/xsa_logs
  manifest_path="$(mktemp /tmp/qwen_xsa_attention_matrix_manifest.XXXXXX)"
...
if jsonl_path:
    ...
    from qwen_xsa_attention_matrix_viz import SHORT_TEXTS
...
PY
  ...
  PARALLEL_LOG_PATH="${PARALLEL_LOG_PATH:-representation-analysis/outputs/xsa_logs/qwen_xsa_attention_matrix_viz-${timestamp}.log}"
  ...
  if [[ -z "${ACTIVE_LOG_PATH:-}" ]]; then
    if is_remote_live_log_path "$PARALLEL_LOG_PATH"; then
      ACTIVE_LOG_PATH="/tmp/qwen_xsa_attention_matrix_viz-${timestamp}.log"
    else
  ...
  : > "$ACTIVE_LOG_PATH"
  fifo_path="$(mktemp -u /tmp/qwen_xsa_attention_matrix_fifo.XXXXXX)"
  mkfifo "$fifo_path"
  ...
  for ((batch_start=0; batch_start<${#manifest_rows[@]}; batch_start+=batch_size_jobs)); do
    ...
    for ((i=batch_start; i<batch_end; ++i)); do
      ...
      log_info "sample_idx=${sample_idx}"
      log_info "Logs:"
      log_info "  $ACTIVE_LOG_PATH"
  ...
  if [[ "${#failed_jobs[@]}" -gt 0 ]]; then
    log_error "One or more attention-matrix jobs failed:"
