...
SWEEP_NAME = "para_ablation"  # "para_ablation" or "perp_ablation"
MODE = "summary_tsv"  # "summary_tsv" or "sweep_dir"
DATA_DIR = REPO_ROOT / "analysis" / "visualization" / "para_ablation" / "data"
INPUT_DIR = REPO_ROOT / "runs" / "analysis" / SWEEP_NAME
SUMMARY_TSV = DATA_DIR / ("perp_ppl_summary.tsv" if SWEEP_NAME == "perp_ablation" else "para_ppl_summary.tsv")
OUTPUT_DIR = REPO_ROOT / "results" / "figures" / SWEEP_NAME
...
def load_summary_tsv(path: Path) -> List[Dict]:
    ...
    parsed = []
    for row in rows:
        axis_key = "perp_scale" if "perp_scale" in row else "para_scale"
        ...
        rec = {
            axis_key: float(row[axis_key]),
            "n_prompts": int(float(row["n_prompts"])),
            "avg_baseline_ppl": value("mean_per_text_baseline_ppl", "avg_baseline_ppl"),
            "avg_xsa_ppl": value("mean_per_text_xsa_ppl", "avg_xsa_ppl"),
            "avg_delta_ppl": value("mean_per_text_delta_ppl_xsa", "avg_delta_ppl_xsa"),
            "avg_residual_attn_ppl": value("mean_per_text_residual_attn_ppl", "avg_residual_attn_ppl"),
            "avg_delta_ppl_residual_attn": value("mean_per_text_delta_ppl_residual_attn", "avg_delta_ppl_residual_attn"),
            "avg_residual_mlp_ppl": value("mean_per_text_residual_mlp_ppl", "avg_residual_mlp_ppl"),
            "avg_delta_ppl_residual_mlp": value("mean_per_text_delta_ppl_residual_mlp", "avg_delta_ppl_residual_mlp"),
            "avg_residual_both_ppl": value("mean_per_text_residual_both_ppl", "avg_residual_both_ppl"),
            "avg_delta_ppl_residual_both": value("mean_per_text_delta_ppl_residual_both", "avg_delta_ppl_residual_both"),
            "path": str(path),
        }
        parsed.append(rec)
    return parsed
...
def plot_generation_length(generation_rows: List[Dict], axis_key: str, output_path: Path) -> None:
    ...
    ax.grid(alpha=0.25)
    ax.legend(frameon=True, facecolor="white", edgecolor="#cfcfcf", title="target")
    fig.tight_layout()
    fig.savefig(output_path, dpi=180)
    plt.close(fig)
...
def run_summary_tsv_mode() -> None:
    ...
    axis_key = choose_sweep_axis(compare_rows)
