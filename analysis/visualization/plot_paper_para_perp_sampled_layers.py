...
# ===== config =====
PAPER_ROOT = Path(__file__).absolute().parents[2]
...
N_SAMPLED_LAYERS = 6
OUT_DIR = PAPER_ROOT / "results" / "figures" / "para_dist"
# ==================
...
def resolve_generation_result_path(model_name: str) -> Path:
    """Prefer the canonical generation trace, but fall back to populated local copies.

    The repo still carries a 1-byte placeholder under results/generation_hidden/ for one
    historical model/output combination. When that file is empty, use the real trace that
    lives in results/generation_probe.json so the paper figure can still be regenerated.
    """

    candidate_paths = [
        build_result_path(ROOT, RUN_TYPE, SPACE, model_name, RANDOM_INIT, MODE, MAX_NEW_TOKENS),
        ROOT / "generation_probe.json",
        ROOT / "generation_probe" / "generation_probe.json",
    ]
...
def plot_one_model(model_name: str) -> None:
    ...
    result_path = resolve_generation_result_path(model_name)
