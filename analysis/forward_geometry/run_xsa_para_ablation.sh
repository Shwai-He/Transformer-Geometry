...
if [[ -n "$PROMPTS_PATH" ]]; then
  ...
  if [[ ! -s "$EFFECTIVE_PROMPTS_PATH" ]]; then
    ...
    "$PYTHON_BIN" "$ROOT_DIR/analysis/utils/prepare_text_dataset_jsonl.py" \
...
PY
  ...
  SCALE_DIR="$OUT_DIR/layer_scales/para_scale_${para_scale//./p}"
  "$PYTHON_BIN" "$SCRIPT_DIR/write_xsa_layer_scales.py" \
    ...
    --default_para_scale "$para_scale" \
    --tag "para_scale=$para_scale"
