...
def _remove_parallel_and_stats(
    ...
    perp_scale: float | torch.Tensor = 1.0,
):
    y_f = y.float()
    r_f = ref.float()
    ...
    perp_value = _scale_like_batch(perp_scale, perp) if isinstance(perp_scale, torch.Tensor) else float(perp_scale)
    y_new = perp_value * perp + (1.0 - alpha_value) * proj
...
def _sample_layer_scales(
    *,
    ...
    perp_scale_min: float,
    perp_scale_max: float,
):
    ...
    for layer_idx in range(n_layers):
        ...
        layer_perp_scale = float(perp_scale)
        if mode in {"layer_para_uniform", "layer_both_uniform"}:
            para_scale = rng.uniform(float(para_scale_min), float(para_scale_max))
        if mode in {"layer_para_choice", "layer_both_choice"}:
            para_scale = rng.choice([float(para_scale_min), float(para_scale_max)])
        if mode in {"layer_perp_uniform", "layer_both_uniform"}:
            layer_perp_scale = rng.uniform(float(perp_scale_min), float(perp_scale_max))
        if mode in {"layer_perp_choice", "layer_both_choice"}:
            layer_perp_scale = rng.choice([float(perp_scale_min), float(perp_scale_max)])
        layer_alpha[layer_idx] = 1.0 - para_scale
        layer_perp[layer_idx] = layer_perp_scale
    return layer_alpha, layer_perp


def _sample_batch_scale(
    *,
    mode: str,
    rng: random.Random,
    ref: torch.Tensor,
    default_alpha: float,
    default_perp_scale: float,
    para_scale_min: float,
    para_scale_max: float,
    perp_scale_min: float,
    perp_scale_max: float,
    ...
    mode = str(mode).lower().strip()
    if not mode.startswith("sample_") or ref.dim() == 0:
        return float(default_alpha), float(default_perp_scale)
    batch = int(ref.shape[0])
    para_scale = None
    perp_scale = None
    if mode in {"sample_para_uniform", "sample_both_uniform"}:
        para_scale = [rng.uniform(float(para_scale_min), float(para_scale_max)) for _ in range(batch)]
    if mode in {"sample_para_choice", "sample_both_choice"}:
        para_scale = [rng.choice([float(para_scale_min), float(para_scale_max)]) for _ in range(batch)]
    if mode in {"sample_perp_uniform", "sample_both_uniform"}:
        perp_scale = [rng.uniform(float(perp_scale_min), float(perp_scale_max)) for _ in range(batch)]
    if mode in {"sample_perp_choice", "sample_both_choice"}:
        perp_scale = [rng.choice([float(perp_scale_min), float(perp_scale_max)]) for _ in range(batch)]
    if para_scale is None and perp_scale is None:
        raise ValueError(f"Unsupported sample scale mode={mode!r}")
    view_shape = (batch,) + (1,) * (ref.dim() - 1)
    alpha = float(default_alpha)
    if para_scale is not None:
        alpha = torch.tensor([1.0 - scale for scale in para_scale], device=ref.device, dtype=torch.float32).reshape(view_shape)
    perp = float(default_perp_scale)
    if perp_scale is not None:
        perp = torch.tensor(perp_scale, device=ref.device, dtype=torch.float32).reshape(view_shape)
    return alpha, perp
...
class QwenXSAForwardHooks:
    ...
    def __init__(
        ...
        xsa_layer_perp_scale_min: float = -1.5,
        xsa_layer_perp_scale_max: float = 1.5,
        ...
        if self.xsa_layer_scale_mode not in valid_layer_scale_modes:
            raise ValueError(
                ...
                f"{'/'.join(sorted(valid_layer_scale_modes))}, "
                f"got {xsa_layer_scale_mode}"
            )
        ...
        self.xsa_layer_perp_scale_min = float(xsa_layer_perp_scale_min)
        self.xsa_layer_perp_scale_max = float(xsa_layer_perp_scale_max)
        ...
        self.layer_alpha, self.layer_perp_scale = _sample_layer_scales(
            ...
            perp_scale_min=self.xsa_layer_perp_scale_min,
            perp_scale_max=self.xsa_layer_perp_scale_max,
        )
        ...
        self.layer_window = {
            ...
            "xsa_layer_perp_scale_min": self.xsa_layer_perp_scale_min,
            "xsa_layer_perp_scale_max": self.xsa_layer_perp_scale_max,
            "xsa_layer_para_scales": {
                str(idx): 1.0 - self.layer_alpha[idx] for idx in sorted(self.active_layer_indices)
            },
            "xsa_layer_perp_scales": {
                str(idx): self.layer_perp_scale[idx] for idx in sorted(self.active_layer_indices)
            },
    ...
    def _scale_for_layer(
        self, layer_idx: int, ref: Optional[torch.Tensor] = None
    ) -> tuple[float | torch.Tensor, float | torch.Tensor]:
        if self.xsa_layer_scale_mode.startswith("sample_") and isinstance(ref, torch.Tensor):
            return _sample_batch_scale(
                mode=self.xsa_layer_scale_mode,
                rng=self.sample_rng,
                ref=ref,
                default_alpha=self.xsa_alpha,
                default_perp_scale=self.xsa_perp_scale,
                para_scale_min=self.xsa_layer_para_scale_min,
                para_scale_max=self.xsa_layer_para_scale_max,
                perp_scale_min=self.xsa_layer_perp_scale_min,
                perp_scale_max=self.xsa_layer_perp_scale_max,
        ...
        return (
            float(self.layer_alpha.get(layer_idx, self.xsa_alpha)),
            float(self.layer_perp_scale.get(layer_idx, self.xsa_perp_scale)),
        )

    def _alpha_for_layer(self, layer_idx: int) -> float:
        return float(self.layer_alpha.get(layer_idx, self.xsa_alpha))

    def _perp_scale_for_layer(self, layer_idx: int) -> float:
        return float(self.layer_perp_scale.get(layer_idx, self.xsa_perp_scale))
                ...
                if mixer_kind == "full_attention" and self.intervention_site == "residual_output":
                    ...
                    if v_proj is not None and o_proj is not None:
                        ...
                        def o_proj_stats_pre_hook(_mod, args, kwargs, _idx=layer_idx, _attn=mixer_module):
                            ...
                            if ref is None or ref.shape != y_pre.shape:
                                return None
                            alpha, perp_scale = self._scale_for_layer(_idx, ref)
                            _, stats = _remove_parallel_and_stats(
                                y_pre, ref, alpha=alpha, perp_scale=perp_scale
                            )
                    ...
                    def attn_hook(_mod, args, kwargs, output, _idx=layer_idx):
                        ...
                        if not isinstance(attn_out, torch.Tensor) or attn_out.shape != residual.shape:
                            return output
                        alpha, perp_scale = self._scale_for_layer(_idx, residual)
                        new_attn, stats = _remove_parallel_and_stats(
                            attn_out, residual, alpha=alpha, perp_scale=perp_scale
                        )
                    ...
                    def o_proj_pre_hook(_mod, args, kwargs, _idx=layer_idx, _attn=mixer_module):
                        ...
                        if self.intervention_site == "xsa_middle":
                            ...
                            new_y_pre, stats = _remove_parallel_attn_multihead(
                                y_pre, ref, _attn, alpha=alpha, perp_scale=perp_scale
                            )
                        self._record_stats("attn", _idx, stats)
                        self._record_stats("attn_pre_o_proj", _idx, stats)
                        self._record_stats("token_mixer", _idx, stats)
                    ...
                    if qkv_proj is not None and out_proj is not None:
                        ...
                        def linear_out_proj_stats_pre_hook(_mod, args, kwargs, _idx=layer_idx):
                            ...
                            if value.shape != y_pre.shape:
                                return None
                            alpha, perp_scale = self._scale_for_layer(_idx, value)
                            _, stats = _remove_parallel_and_stats(
                                y_pre, value, alpha=alpha, perp_scale=perp_scale
                            )
                    ...
                    def linear_hook(_mod, args, kwargs, output, _idx=layer_idx):
                        ...
                        new_linear, stats = _remove_parallel_and_stats(
                            linear_out, residual, alpha=alpha, perp_scale=perp_scale
                        )
            ...
            if self.target in {"mlp", "both"}:
                ...
                def mlp_hook(_mod, args, kwargs, output, _idx=layer_idx):
                    ...
                    new_mlp, stats = _remove_parallel_and_stats(
                        mlp_out, residual, alpha=alpha, perp_scale=perp_scale
                    )
                    for name, value in stats.items():
...
def run_target(model, tokenizer, texts: List[str], target: str, args) -> Dict:
    ...
    with QwenXSAForwardHooks(
        ...
        xsa_layer_scale_mode=args.xsa_layer_scale_mode,
        xsa_layer_scale_seed=args.xsa_layer_scale_seed,
        xsa_layer_para_scale_min=args.xsa_layer_para_scale_min,
        xsa_layer_para_scale_max=args.xsa_layer_para_scale_max,
        xsa_layer_perp_scale_min=args.xsa_layer_perp_scale_min,
        xsa_layer_perp_scale_max=args.xsa_layer_perp_scale_max,
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help="Optionally sample per-layer or per-forward-sample para/perp scales with a fixed seed.",
    )
    parser.add_argument("--xsa_layer_scale_seed", type=int, default=0)
    parser.add_argument("--xsa_layer_para_scale_min", type=float, default=-20.0)
    parser.add_argument("--xsa_layer_para_scale_max", type=float, default=20.0)
    parser.add_argument("--xsa_layer_perp_scale_min", type=float, default=-1.5)
    parser.add_argument("--xsa_layer_perp_scale_max", type=float, default=1.5)
    parser.add_argument(
        "--xsa_track_stats",
    ...
    results = {
        ...
        "xsa_layer_scale_mode": args.xsa_layer_scale_mode,
        "xsa_layer_scale_seed": int(args.xsa_layer_scale_seed),
        "xsa_layer_para_scale_min": float(args.xsa_layer_para_scale_min),
        "xsa_layer_para_scale_max": float(args.xsa_layer_para_scale_max),
        "xsa_layer_perp_scale_min": float(args.xsa_layer_perp_scale_min),
        "xsa_layer_perp_scale_max": float(args.xsa_layer_perp_scale_max),
