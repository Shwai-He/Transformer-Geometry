...
echo "[INFO] XSA_LAYER_PERP_SCALE_MIN=$XSA_LAYER_PERP_SCALE_MIN"
echo "[INFO] XSA_LAYER_PERP_SCALE_MAX=$XSA_LAYER_PERP_SCALE_MAX"
echo "[INFO] XSA_PRIMARY_SITE=$XSA_PRIMARY_SITE"
echo "[INFO] XSA_VALUE_REF_EXPANSION=$XSA_VALUE_REF_EXPANSION"
echo "[INFO] XSA_ATTN_ATTR_SOURCE=$XSA_ATTN_ATTR_SOURCE"
...
if prompts_path:
    ...
    from analysis.forward_geometry.qwen_xsa_forward_ablation import BUILTIN_TEXTS
...
if [[ "${STREAM_FORWARD_ABLATION_LOGS,,}" == "true" ]]; then
  ...
  XSA_LAYER_PERP_SCALE_MIN="$XSA_LAYER_PERP_SCALE_MIN" \
  XSA_LAYER_PERP_SCALE_MAX="$XSA_LAYER_PERP_SCALE_MAX" \
  XSA_INTERVENTION_SITES="$XSA_INTERVENTION_SITES" \
  XSA_VALUE_REF_EXPANSION="$XSA_VALUE_REF_EXPANSION" \
  XSA_ATTN_ATTR_SOURCE="$XSA_ATTN_ATTR_SOURCE" \
  XSA_MIDDLE_TARGETS="$XSA_MIDDLE_TARGETS" \
  RESIDUAL_OUTPUT_TARGETS="$RESIDUAL_OUTPUT_TARGETS" \
  DTYPE="$DTYPE" \
  ...
  bash "$SCRIPT_DIR/run_xsa_forward_ablation.sh" 2>&1 | tee "$forward_log_path"
else
  MODEL_NAME="$MODEL_NAME" \
  JSONL_PATH="$prepared_prompts_path" \
  TEXT_KEY="text" \
  ...
  OUTPUT_JSON="$forward_output_json" \
  PER_TEXT_OUTPUT_JSONL="$per_text_output_jsonl" \
  bash "$SCRIPT_DIR/run_xsa_forward_ablation.sh" >"$forward_log_path" 2>&1
fi
...
primary_site = os.environ.get("XSA_PRIMARY_SITE", "xsa_middle_multihead")
...
with per_text_path.open("r", encoding="utf-8") as f:
    for line in f:
        ...
        xsa_site = payload["sites"][primary_site]["attn"]
