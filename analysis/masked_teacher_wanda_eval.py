...
def main():
    ...
    args.output_json = (
        ...
        f"masked-teacher-wanda-{wanda_sparsity}-{wanda_method}.json"
    )
    ...
    dense_path = args.dense_model_name
    dropped_path = args.dropped_model_name
    if not os.path.isdir(dropped_path):
        raise FileNotFoundError(f"Dropped(Wanda) model path not found: {dropped_path}")

    dense_tokenizer = AutoTokenizer.from_pretrained(dense_path, trust_remote_code=True)
    if dense_tokenizer.pad_token is None:
        dense_tokenizer.pad_token = dense_tokenizer.eos_token
    dense_model = AutoModelForCausalLM.from_pretrained(dense_path, trust_remote_code=True).to(device).eval()

    dropped_tokenizer = AutoTokenizer.from_pretrained(dropped_path, trust_remote_code=True)
    if dropped_tokenizer.pad_token is None:
        dropped_tokenizer.pad_token = dropped_tokenizer.eos_token
    dropped_model = AutoModelForCausalLM.from_pretrained(dropped_path, trust_remote_code=True).to(device).eval()
