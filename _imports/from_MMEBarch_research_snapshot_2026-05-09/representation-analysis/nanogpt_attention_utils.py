...
def load_nanogpt_checkpoint(ckpt_path: str, device: str, dtype: str, nanogpt_repo_root: Optional[str] = None):
    ...
    checkpoint = torch.load(str(ckpt_file), map_location=device, weights_only=False)
    ...
    if benign_unexpected or incompatible.missing_keys:
        print(
            f"[INFO] Load complete with known arch differences — "
            f"ignored unexpected={benign_unexpected}; "
            f"randomly-init missing={list(incompatible.missing_keys)}"
        )
