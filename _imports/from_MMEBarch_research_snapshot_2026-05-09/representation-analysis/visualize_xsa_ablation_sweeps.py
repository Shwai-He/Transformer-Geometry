...
SUMMARY_TSV = Path("/Users/bytedance/Documents/GitHub/MMEBarch/representation-analysis/outputs/Qwen3-4B/para_ppl_summary.tsv")
...
def load_summary_tsv(path: Path) -> List[Dict]:
    ...
    for row in rows:
        rec = {
            "para_scale": float(row["para_scale"]),
            "n_prompts": int(float(row["n_prompts"])),
            "avg_baseline_ppl": float(row["avg_baseline_ppl"]),
            "avg_xsa_ppl": float(row["avg_xsa_ppl"]),
            "avg_delta_ppl": float(row["avg_delta_ppl_xsa"]),
            "avg_residual_attn_ppl": float(row["avg_residual_attn_ppl"]),
            "avg_delta_ppl_residual_attn": float(row["avg_delta_ppl_residual_attn"]),
            "avg_residual_mlp_ppl": float(row["avg_residual_mlp_ppl"]),
            "avg_delta_ppl_residual_mlp": float(row["avg_delta_ppl_residual_mlp"]),
            "avg_residual_both_ppl": float(row["avg_residual_both_ppl"]),
            "avg_delta_ppl_residual_both": float(row["avg_delta_ppl_residual_both"]),
            "path": str(path),
        }
        parsed.append(rec)
    return parsed
