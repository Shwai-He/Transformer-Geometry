...
def compute_effective_diag_delta(attn_probs: torch.Tensor, value: torch.Tensor, attn_module) -> torch.Tensor:
    if attn_probs.dim() != 4:
...
def run_single_flip(
    ...
    flip_raw_diag = torch.diagonal(flip_xsa)
    flip_raw_offdiag_sum = flip_xsa.sum(dim=-1) - flip_raw_diag
    flip_raw_total_sum = flip_xsa.sum(dim=-1)
    effective_compare = baseline_layer[0, flip_head] + effective_delta_layer[0, flip_head] if effective_delta_layer is not None else None
    effective_diag = torch.diagonal(effective_compare) if effective_compare is not None else None
    effective_offdiag_sum = effective_compare.sum(dim=-1) - effective_diag if effective_compare is not None else None
    effective_total_sum = effective_compare.sum(dim=-1) if effective_compare is not None else None

    plot_bundle_json_path = _save_plot_bundle_json(
        plot_bundle_path,
        baseline_attention_layer=baseline_layer[0],
        raw_xsa_attention_layer=xsa_layer[0],
        effective_delta_layer=effective_delta_layer[0] if effective_delta_layer is not None else None,
        panel1_flip_head_baseline=flip_baseline,
        panel2_flip_head_effective_compare=effective_compare,
        panel2_flip_head_raw_xsa=flip_xsa,
        panel3_flip_head_effective_delta=effective_delta_layer[0, flip_head] if effective_delta_layer is not None else None,
        panel4_raw_diag=flip_raw_diag,
        panel4_raw_offdiag_sum=flip_raw_offdiag_sum,
        panel4_raw_total_sum=flip_raw_total_sum,
        panel4_effective_diag=effective_diag,
        panel4_effective_offdiag_sum=effective_offdiag_sum,
        panel4_effective_total_sum=effective_total_sum,
    ...
    payload = {
        ...
        "token_mixer_kind": mixer_kind,
        "note": (
            "Single-layer XSA flip view. plot_bundle.json stores raw values before any plotting color remapping."
        ),
    ...
    print(f"[INFO] flip_layer={flip_layer} saved plot bundle to {plot_bundle_json_path}")
    return payload
    ...
    def run_one_sample(sample_idx: int, raw_text: str) -> Dict[str, Any]:
        ...
        if int(args.flip_layer) == -1:
            ...
            for flip_layer in range(n_layers):
                layer_bundle_path = sweep_dir / f"layer_{flip_layer:02d}_plot_bundle.json"
                if layer_bundle_path.exists() and not bool(args.overwrite_existing):
                    print(
                        f"[SKIP] sample_idx={sample_idx} layer={flip_layer} reusing existing plot bundle: {layer_bundle_path}",
                        flush=True,
                    )
                    all_payloads.append({"sample_idx": sample_idx, "flip_layer": flip_layer, "plot_bundle_path": str(layer_bundle_path)})
                    continue
                all_payloads.append(
                    run_single_flip(
                        model=model,
