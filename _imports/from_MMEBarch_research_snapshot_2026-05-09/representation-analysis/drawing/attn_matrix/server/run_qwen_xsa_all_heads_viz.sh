...
DATA_TAG="$(sanitize_tag "${JSONL_PATH:-${PROMPT_SET}_builtin}")"
...
OUTPUT_PREFIX="${OUTPUT_PREFIX:-${BASE_OUTPUT_DIR}/qwen_xsa_all_heads-data${DATA_TAG}-sample${SAMPLE_IDX}-flip${FLIP_LAYER}-${CHAT_TAG}-site${SITE_TAG}-dtype${DTYPE_TAG}-dev${DEVICE_TAG}}"
...
build_head_list
MANIFEST_JSONL="$(mktemp /tmp/qwen_xsa_all_heads_entries.XXXXXX)"
...
launch_batch() {
  ...
  for ((i=0; i<batch_size; ++i)); do
    ...
    if skip_path="$(should_skip_head "$head")"; then
      ...
      local stem
      stem="$(head_stem "$head")"
      append_manifest_entry "$head" "skipped_existing" "$skip_path" "${stem}/selected_layers/layer00_flip.png" ""
      continue
    fi
  ...
  for ((j=0; j<${#pids[@]}; ++j)); do
    ...
    stem="$(head_stem "$head")"
    local bundle_path="${stem}/plot_bundle.json"
    local image_path="${stem}/selected_layers/layer00_flip.png"
    local log_path="$RUN_DIR/head$(printf '%02d' "$head").log"
...
"$PYTHON_BIN" - <<'PY' "$MANIFEST_JSONL" "$INDEX_PATH" "$MODEL_NAME" "$SAMPLE_IDX" "$FLIP_LAYER" "$XSA_INTERVENTION_SITE"
import json, sys
from pathlib import Path
...
payload = {
    ...
    "xsa_intervention_site": sys.argv[6],
    "runs": [],
}
