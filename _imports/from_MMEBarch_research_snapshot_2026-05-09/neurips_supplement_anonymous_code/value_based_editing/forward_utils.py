    ...
    def _attach_layer_hooks(self, layer, layer_idx):
        def layer_pre_hook(_module, inputs, kwargs):
            if self.current_step is None:
                return
            raw = inputs[0] if inputs else kwargs.get("hidden_states")
            hidden = _to_last_token(raw) if raw is not None else None
...
def generate_with_custom_forward(
    ...
    try:
        for _ in range(max_new_tokens):
            ...
            generated_probs.append(F.softmax(last_logits, dim=-1))
