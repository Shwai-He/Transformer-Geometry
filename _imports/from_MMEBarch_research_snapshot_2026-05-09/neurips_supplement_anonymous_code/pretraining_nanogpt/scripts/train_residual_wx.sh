...
# Usage example:
#   bash scripts/train_residual_wx.sh config/train_gpt2.py
...
RESIDUAL_WX_LEARNABLE="${RESIDUAL_WX_LEARNABLE:-true}"
...
WANDB_RUN_NAME="${WANDB_RUN_NAME:-residual-wx-rlr${RESIDUAL_LR_RATIO}-xobj${XSA_SELF_LOSS_OBJECTIVE}-s${SEED}}"
OUT_DIR="${OUT_DIR:-out/residual-wx-rlr${RESIDUAL_LR_RATIO}-xobj${XSA_SELF_LOSS_OBJECTIVE}-s${SEED}}"
...
echo "[INFO] Running residual (wx)... (nproc_per_node=$NPROC_PER_NODE_RESOLVED)"
run_training_once() {
  local -a run_cmd=("$@")
