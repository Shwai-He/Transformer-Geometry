...
# Usage example:
#   RESIDUAL_MODE=param bash scripts/train_residual.sh config/train_gpt2.py
...
RESIDUAL_MODE="${RESIDUAL_MODE:-param}" # param | wx
RESIDUAL_ATTN_PARAM="${RESIDUAL_ATTN_PARAM:-1.0}"
RESIDUAL_MLP_PARAM="${RESIDUAL_MLP_PARAM:-1.0}"
RESIDUAL_WX="${RESIDUAL_WX:-1.0}"
RESIDUAL_WX_LEARNABLE="${RESIDUAL_WX_LEARNABLE:-false}"
RESIDUAL_SCALE_LEARNABLE="${RESIDUAL_SCALE_LEARNABLE:-false}"
RESIDUAL_SCALE_INIT="${RESIDUAL_SCALE_INIT:-1.0}"
RESIDUAL_SCALE_DEPTH_SLOPE="${RESIDUAL_SCALE_DEPTH_SLOPE:-0.0}"
RESIDUAL_LR_RATIO="${RESIDUAL_LR_RATIO:-1.0}"
USE_POST_NORM="${USE_POST_NORM:-false}"
POST_NORM_START_LAYER="${POST_NORM_START_LAYER:-1000000000}"
WANDB_RUN_NAME="${WANDB_RUN_NAME:-residual-${RESIDUAL_MODE}-rlr${RESIDUAL_LR_RATIO}-xobj${XSA_SELF_LOSS_OBJECTIVE}-s${SEED}}"
OUT_DIR="${OUT_DIR:-out/residual-${RESIDUAL_MODE}-rlr${RESIDUAL_LR_RATIO}-xobj${XSA_SELF_LOSS_OBJECTIVE}-s${SEED}}"
...
echo "[INFO] Running residual (${RESIDUAL_MODE})... (nproc_per_node=$NPROC_PER_NODE_RESOLVED)"
run_training_once() {
  local -a run_cmd=("$@")
  ...
  local residual_wx_learnable_py residual_scale_learnable_py use_post_norm_py
  residual_wx_learnable_py="$(to_python_bool "$RESIDUAL_WX_LEARNABLE")"
  residual_scale_learnable_py="$(to_python_bool "$RESIDUAL_SCALE_LEARNABLE")"
  use_post_norm_py="$(to_python_bool "$USE_POST_NORM")"
  local wandb_log_py
  ...
  --residual_wx="$RESIDUAL_WX" \
  --residual_wx_learnable="$residual_wx_learnable_py" \
  --residual_scale_learnable="$residual_scale_learnable_py" \
  --residual_scale_init="$RESIDUAL_SCALE_INIT" \
  --residual_scale_depth_slope="$RESIDUAL_SCALE_DEPTH_SLOPE" \
  --residual_lr_ratio="$RESIDUAL_LR_RATIO" \
  --use_post_norm="$use_post_norm_py" \
  --post_norm_start_layer="$POST_NORM_START_LAYER"
}
