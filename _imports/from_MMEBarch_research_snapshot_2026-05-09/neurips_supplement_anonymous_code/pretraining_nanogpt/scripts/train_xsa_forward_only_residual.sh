...
# Usage example:
#   bash scripts/train_xsa_forward_only_residual.sh config/train_gpt2.py
...
XSA_FORWARD_ONLY="${XSA_FORWARD_ONLY:-true}"
XSA_FORWARD_TARGET="${XSA_FORWARD_TARGET:-attn}" # attn | both | mlp
XSA_FORWARD_REF="${XSA_FORWARD_REF:-residual}" # self_value (strict XSA) | residual (legacy)
...
WANDB_RUN_NAME="${WANDB_RUN_NAME:-${RUN_NAME_PREFIX}xsa-forward-only-residual-xfr${XSA_FORWARD_REF}-xsp${XSA_FORWARD_SPACE}-xft${XSA_FORWARD_TARGET}-xobj${XSA_SELF_LOSS_OBJECTIVE}-s${SEED}}"
OUT_DIR="${OUT_DIR:-out/${RUN_NAME_PREFIX}xsa-forward-only-residual-xfr${XSA_FORWARD_REF}-xsp${XSA_FORWARD_SPACE}-xft${XSA_FORWARD_TARGET}-xobj${XSA_SELF_LOSS_OBJECTIVE}-s${SEED}}"
