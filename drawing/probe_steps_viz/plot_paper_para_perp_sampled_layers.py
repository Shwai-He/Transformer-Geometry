...
def main() -> None:
    ...
    for i, (ax, item) in enumerate(zip(axes, PLOT_DATA)):
        ...
        ax.text(0.5, -0.18, f"L{{item['layer']}}", transform=ax.transAxes, fontsize=10, ha="center", va="top")
        if i == 0:
    ...
    fig.supxlabel("Generation step", fontsize=11, y=0.02)
...
def plot_one_model(model_name: str) -> None:
    ...
    for i, (ax, layer) in enumerate(zip(axes, layers)):
        ...
        ax.text(0.5, -0.18, f"L{layer}", transform=ax.transAxes, fontsize=10, ha="center", va="top")
        if i == 0:
            ax.set_ylabel(r"$\|\Delta_{\parallel}\| / \|\Delta_{\perp}\|$", fontsize=10)
        ax.grid(alpha=0.22)
